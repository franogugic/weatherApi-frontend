...
export function AvailableLocationsPanel({
  ...
  return (
    <div className="flex max-h-[460px] min-h-0 flex-col rounded-[28px] bg-div p-4 sm:p-6 lg:rounded-4xl xl:h-full xl:max-h-none xl:w-[320px] xl:shrink-0">
      ...
      <ul className="min-h-0 flex-1 overflow-y-auto">
        {locations.map((location) => (
          <li key={location.id}>
            <NavLink to={`/${location.id}`} className="block border-y border-white/20 p-3 transition-colors hover:bg-white/8 sm:p-4">
              <div className="mb-2 flex items-center justify-start gap-2">
                <span><MapPin className="text-accent-primary" /></span>
                <p className="min-w-0 break-words text-base font-semibold sm:text-[18px]">{location.name}</p>
              </div>
